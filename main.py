...
def argps():
    ...
    parser.add_argument("--use_mid", help="whether to use entity in the source language, not used",
                        type=str2bool, default=False)

    # type information for entities, not used
    parser.add_argument("--trg_encoding_num", help="how many representations for each target entity, we assume that there are at most 10 aliases for an entity",
                        type=int, default=1)
    parser.add_argument("--mid_encoding_num", type=int, default=1)
    parser.add_argument("--trg_type_idx", help="the column index of target entity type", type=int, default=3)
    parser.add_argument("--mid_type_idx", help="the column index of source entity type", type=int, default=3)
    parser.add_argument("--kb_type_idx", help="the column index of entity type in the KB file", type=int, default=2)
    parser.add_argument("--pivot_type_idx", type=int, default=3)
    parser.add_argument("--alia_file", type=str, default="HOLDER")
    parser.add_argument("--method", choices=("base", "pivoting"), default="base")

    # filter
    parser.add_argument("--n_gram_threshold", help="ignore n gram with less than the min frequency", type=int, default=0)

    # middle stuff, not used
